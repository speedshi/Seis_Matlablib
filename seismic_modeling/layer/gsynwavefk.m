function [wzc,wnc,wec,fzc,fnc,fec]=gsynwavefk(soup,recp,smt,sm0,stf,dt,ENt)
...
% The unit of length in this program are all in 'km'. Because fk use km.
...
% All the input and output data are stored and saved at the current folder by default.
% INPUT-------------------------------------------------------------------------------
% soup: source position. Each row represents a three-dimensional source
% position (column 1: X, column 2: Y, column 3: Z), nsr*3;
% recp: surface geophone positioin. Each row represents a three-dimensional
% geophone position (column 1: X, column 2: Y, column 3: Z), nre*3;
...
% stf: source time functions, nsr*nt, origin time should be already considered in source time function;
...
recp(:,3)=round(recp(:,3),dpre); % set precision of the receiver depth, need to set precision before use 'unique' to pick and order the depth
...
% NOTE for fk, the sources and receivers cannot be at the same depth.
% Here preprocess the receiver positions. If the receivers are at the same
% depth with the sources, the receivers will be replace to a deeper depth.
reprdd=0.001; % distance used to replace the receivers, note it is in km.
for is=1:nsr
    rindx=recp(:,3)==soup(is,3); % obtain the logical array which represents the receivers have the same depth with source
    recp(rindx,3)=recp(rindx,3)+reprdd; % replace the recreivers to a deeper depth
end
...
rdp=unique(recp(:,3)); % find out different depth of receivers, and also sort the depth in ascending order
nrdp=max(size(rdp)); % number of different receiver depth
...
for ii=1:nsdp
    ...
    for ir=1:nrdp
        rid=recp(:,3)==rdp(ir); % index of this particular receiver depth, logical array
        srf1=sroff(sid,rid); % pick out the source-receiver offset of this depth, merge them
        srf1=srf1(:)';
        srf=unique(srf1); % only save the different source-reveiver offsets, delete the same offsets of this depth
        srez=num2str(rdp(ir),sftp); % transfer receiver depth to string
        ssrf=num2str(srf,sftps); % transfer source-receiver offsets to string, note the 'blank space' is very important for separating the different numbers
...
for is=1:nsr
    ...
    for ir=1:nre
        razi=num2str(srazi(is,ir)); % transfer the source-receiver azimuth to string. For azimuth in degree, using default precision (4 number after decimal point) is enough
        ssrf=num2str(sroff(is,ir),sftp); % transfer this source-receiver offset to string. Note only one offest, so no need to add 'blank space' here
        srez=num2str(recp(ir,3),sftp); % transfer receiver depth to string
...
for is=1:nsr
    for ir=1:nre
        ...
        % only keep the useful data (1-Nt) for each trace
        tdzc=cvzc(1:Nt); % Z component
        tdnc=cvnc(1:Nt); % N component
        tdec=cvec(1:Nt); % E component
