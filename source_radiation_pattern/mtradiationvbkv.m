...
% The function have been adapted to plot a cubic figure.
% P-wave adjust version, make a cubic plot, an exact sphere for P-wave radiation pattern
% note that: 'adcoe' need to be adjusted when apply a different 'AutoScaleFactor' for P-wave plot
% and for different input moment tensors
% here when 'AutoScaleFactor' is 2.6 and m11=1 m33=-1, 'adcoe' is 0.33
% INPUT--------------------------------------------------
...
colormap(cmapmtv);
adcoe=0.33; % need adjust this coefficient to match the sphere when apply different 'ScaleFactor' for P-wave
for jj=1:nc
    for ii=1:nr
        if cpm(ii,jj)>0
            rdad=1-adcoe*sqrt(fpr1(ii,jj)^2+fpr2(ii,jj)^2+fpr3(ii,jj)^2); 
            rnx(ii,jj)=rdad*sin(gtheta(ii,jj))*cos(gphi(ii,jj));
            rny(ii,jj)=rdad*sin(gtheta(ii,jj))*sin(gphi(ii,jj));
            rnz(ii,jj)=rdad*cos(gtheta(ii,jj));
        end
    end
end
