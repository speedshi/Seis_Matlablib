...
if mcm.phasetp==0
    % only use P-phase
    time_end=(size(trace.data,2)-1)*trace.dt-max(trace.travelp(:))-mcm.tpwind;
elseif mcm.phasetp==1
    % only use S-phase
    time_end=(size(trace.data,2)-1)*trace.dt-max(trace.travels(:))-mcm.tswind;
elseif mcm.phasetp==2
    % use both P- and S-phase
    maxwind=max(mcm.tpwind,mcm.tswind);
    time_end=(size(trace.data,2)-1)*trace.dt-max(trace.travels(:))-maxwind;
else
