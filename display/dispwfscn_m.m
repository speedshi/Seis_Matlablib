function dispwfscn_m(data,recp,soup,dt,stos,travelp,travels,name,t0,pap)
...
% recorded data, in second; can be empty.
% travelp: the travel time of P-wave from this event to different stations,
% shape: 1*nre, different column corresponding to different stations, in
% second; can be empty.
% travels: the travel time of S-wave from this event to different stations,
% shape: 1*nre, different column corresponding to different stations, in
% second; can be empty.
% name: names of the stations, cell array, shape: nre*1; can be empty.
% t0: matlab datetime, the origin time of the seismic data; can be empty.
...
for ire=1:nre
    ...
    if ~isempty(travelp) && ~isempty(stos)
        ...
        plot(xtimes,ofsda(ire),'bx','linewidth',1.2); hold on; % mark the calculated P-wave arrival time
    end
    if ~isempty(travels) && ~isempty(stos)
        ...
        plot(xtimes,ofsda(ire),'rx','linewidth',1.2); hold on; % mark the calculated S-wave arrival time
    end
end
axis tight; 
set(gca,'ytick',ofsda,'yticklabel',name(idf)); % display the correct distance label
