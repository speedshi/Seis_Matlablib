function dispwfsc(data,recp,soup,dt,stos,travelp,travels,pap)
% This function is used to plot the waveform section according to the
% offsets between the event and stations.
% Travel times are marked directly on the recorded waveforms.
% Input:----------------------------------------------------
...
for ire=1:nre
    ...
    plot((travelp(idf(ire))+stos),dext(round((travelp(idf(ire))+stos)/dt)+2-nt1),'bx','linewidth',1.2); hold on; % mark the calculated P-wave arrival time
    plot((travels(idf(ire))+stos),dext(round((travels(idf(ire))+stos)/dt)+2-nt1),'rx','linewidth',1.2); hold on; % mark the calculated S-wave arrival time
end
